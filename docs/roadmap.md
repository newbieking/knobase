...
状态：**部分完成**（2026-09-17 主体落地，2026-09-20 审查修正，2026-09-22 级联模型重排 + 真实网关实测）。
DoD 的 paraphrase +10pp 在离线 LSI 下未达，接入真实 embedding 网关后达成（见 S2.2）。下面是立项时的
计划原文，与实测结论有出入处以 [S2 结论](#s2-结论2026-09-17-实测)、[S2.1](#s21-审查修正2026-09-20)
与 [S2.2](#s22-级联模型重排与真实网关实测2026-09-22) 为准。
...
（`RERANK_CANDIDATES`）交给模型打分，请求体兼容两种协议——DashScope 原生
（`input.query` / `input.documents` / `parameters.top_n`，响应取 `output.results[].relevance_score`）
与 OpenAI 风格（`query` / `documents` / `top_n`）；响应按 `index` 归位并逐条校验（索引唯一且不越界、
...
- Cross-encoder rerank：**已在 S2.2 提前落地**（网关 rerank API，本地 Top-30 进入模型打分，
  `0.70/0.22/0.08` 加权混合降级为 fallback）。剩余工作只有 S2.2 遗留的拒答重校准：真实向量下
  绝对阈值与间隔规则都分不开应拒答与可回答（分布证据见 S2.2），改由本迭代的生成侧拒答兜底，
  检索层不再拟合当前语料。
